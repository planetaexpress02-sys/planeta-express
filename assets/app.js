    ...
    <div class="card">
      ...
      <div class="card-b p0">
        ${vencs.length? `<div class="tbl-wrap"><table class="tbl"><tbody>${vencs.map(x=>`
          <tr class="clickable" onclick="modalVencimento('${x.id}')"><td><b>${esc(x.tipo)}</b>${x.obs?`<div class="muted" style="font-size:12px">${esc(x.obs)}</div>`:''}</td>
          <td class="mono">${fmtD(x.validade)}</td><td>${stBadge(x.validade)}</td></tr>`).join('')}</tbody></table></div>` : emptyState('Sem vencimentos cadastrados.')}
      </div>
    </div>
...
function viewExames(){
  ...
  const cel=(m,tipo)=>{
    ...
    if(!v) return `<td><button class="btn ghost sm" onclick="modalVencimento(null,'motorista','${m.id}','${tipo}')">${svg('plus')} Add</button></td>`;
    const s=situacao(v.validade);
    return `<td class="clickable" onclick="modalVencimento('${v.id}')"><div class="st ${s.cls}">${s.label}</div><div class="muted mono" style="font-size:11.5px;margin-top:3px">${fmtD(v.validade)}</div></td>`;
  };
...
function viewDirecao(){
  ...
  return `
  <div class="banner">${svg('wheel')}<div><b>Direção Defensiva</b><span>Certificado de direção defensiva dos motoristas (válido por 1 ano). Cadastre a validade de cada um.</span></div></div>
  <div class="grid kpis" style="grid-template-columns:repeat(3,1fr);margin-bottom:18px">
    ...
    ${kpi('user','i-amber', DB.motoristas.length-arr.length, 'Sem cadastro','')}
  </div>
  <div class="card"><div class="card-b p0"><div class="tbl-wrap"><table class="tbl">
    <thead><tr><th>Motorista</th><th>Emissão</th><th>Validade</th><th>Situação</th><th class="no-print"></th></tr></thead>
